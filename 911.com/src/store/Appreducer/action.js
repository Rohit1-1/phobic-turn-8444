...
import { GET_CART_DATA_PRODUCT_SUCCESS, GET_PRODUCT_FAILURE, GET_PRODUCT_REQUEST, GET_PRODUCT_SUCCESS } from "./action.type"
...
export const addtoCart=(payload)=>(dispatch)=>{
  ...
  export const getCartdata=()=>(dispatch)=>{
    ...
    dispatch(getProduct_request())
  try {
      return axios.get(`${url}/cart/cartproduct`,{
        headers:{
            authorization :`Bearer ${token}`
        }
      }).then((res)=>{
  ...
  export const updateCart=(payload)=>(dispatch)=>{
   ...
   // console.log(token);
    dispatch(getProduct_request())
  try {
      return axios.patch(`${url}/cart/addmore`,payload,{
        headers:{
            authorization :`Bearer ${token}`
        }
      }).then((res)=>{
  ...
  export const orderplacedd=()=>(dispatch)=>{
   ...
   // console.log(token);
    dispatch(getProduct_request())
  try {
      return axios.delete(`${url}/cart/orderplaced/1`,{
        headers:{
            authorization :`Bearer ${token}`,
        }
